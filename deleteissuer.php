...
<body>
<?php
if (isset($_GET['issuerid'])) { $issuerid = $_GET['issuerid']; }
else { header('location:viewissuers.php'); }
if ($issuerid != '')
{
    ...
    <div id="containerInner">
        <form method="post">
            <div class="form clearfix">
                <div class="formHead">Delete Issuer</div>
                    <div class="formFoot clearfix">
                    <span>Are you sure? you want to delete the above issuer? if yes click delete. Or click  cancel to go back.</span>
                    <input type="submit" name="submit" value="Delete" id="submit">
                    <input type="button" name="cancel" value="Cancel" id="cancel"
                    onclick="window.location='viewissuers.php'">
                </div>
            </div>
        </form>
    </div>
...
{
    $qryDeletePermit="select * from stock where idIssue='$issuerid'";
    ...
    {
        $qry = "delete from issue where idIssue='$issuerid'";
        $result = mysql_query($qry) or die(mysql_error());
    ...
    else
    {
        ...
        <script>
            ...
            $('#responseMsg').html('<img id="addResponseImg" style="visibility: hidden"; src="images/failure.png" width="20">' +
                '<span id="msg">Alert!!! You cannot delete this issuer because he has issued something from stock.</span>');
            $('#responseMsg').show();
            setTimeout(function(){
