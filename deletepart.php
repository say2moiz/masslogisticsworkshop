...
else { header('location:viewparts.php'); }
if ($partid != '')
{
        ...
        <form method="post">
            <div class="form clearfix">
                <div class="formHead">Delete Part</div>
                    ...
                    <span>Are you sure? you want to delete the above part? if yes click delete. Or click  cancel to go back.</span>
                    <input type="submit" name="submit" value="Delete" id="submit">
                    <input type="button" name="cancel" value="Cancel" id="cancel"
                    onclick="window.location='viewparts.php'">
                </div>
            </div>
        </form>
    </div>
...
{
    $qryDeletePermit="select * from purchase where idPart='$partid'";
    ...
    {
        $qry = "delete from part where idPart='$partid'";
        $result = mysql_query($qry) or die(mysql_error());

        $qry = "delete from junctionbp where idPart='$partid'";
        $result = mysql_query($qry) or die(mysql_error());
    ...
    else
    {
        ...
        <script>
            ...
            $('#responseMsg').html('<img id="addResponseImg" style="visibility: hidden"; src="images/failure.png" width="20">' +
                '<span id="msg">Alert!!! You cannot delete this part because you have purchased it.</span>');
            $('#responseMsg').show();
            setTimeout(function(){
