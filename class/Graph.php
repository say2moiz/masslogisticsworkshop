<?php
    class reportdata
    {
        ...
        function addbrand($brandname,$partid,$trackable)
        {
            $brandname=mysql_real_escape_string($brandname);
            $partid=mysql_real_escape_string($partid);
            $trackable=mysql_real_escape_string($trackable);
            $resultArr=array();
            if($brandname!='' && $trackable!='' && $partid!=-1)
            {
            ...
            else if($brandname!='' && $trackable!='' && $partid==-1)
            {
                $qry="select * from brand where brandName='$brandname'";
                $result=mysql_query($qry) or die (mysql_error());
                $numofrows=mysql_num_rows($result);
        ...
        function addpart($partname,$brandid,$trackable,$rackid,$parttype,$partdiscription)
        {
            ...
            $resultArr=array();
            if($partname!='' && $trackable!='' && $brandid!=-1)
            {
            ...
            else if($partname!='' && $trackable!='' && $brandid==-1)
            {
                $qry="select * from Part where partName='$partname'";
                $result=mysql_query($qry) or die (mysql_error());
                $numofrows=mysql_num_rows($result);
